...
PICS = (environ.get('PICS', 'https://i.ibb.co/Vcw3mh3K/photo-2026-04-01-08-32-26-7623705905476927512.jpg')).split()  # Sample pic
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003813513961').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1003725961309'))  # Log channel id (make sure bot is admin)
...
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1003522165861').split()] #(make sure bot is admin)
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1003525105249')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1003219334269')  # Request channel id (make sure bot is admin)
...
# ============================
PAYMENT_QR = environ.get('QR_CODE', 'https://i.ibb.co/1GGFRh9S/photo-2026-02-22-10-54-31-7623705613419151364.jpg')    # QR code image for payments
...
DATABASE_NAME = environ.get('DATABASE_NAME', "Cluster0xxz") # Database name (default: cluster)
...
LOG_VR_CHANNEL = int(environ.get('LOG_VR_CHANNEL', '-1003725961309')) #Verification Channel Id 
